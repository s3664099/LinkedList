...
public class SimpleLinkedList implements MyList
{
    ...
    public boolean remove(int value) {
        ...
        //Creates a value for the previous node and the next node to the next node
        Node nextNode;
        Node previousNode = null;
        
        //searches the list looking for the value
        for (int i = 0; i < mLength; ++i) {
        	
        	//Checks to see if the values match
        	if (currNode.getValue() == value) {

        		//checks to see if this is the first node (ie, it equals the head node)
        		if(currNode.equals(mHead))
        		{
        			//sets the next node as the head
        			mHead = currNode.getNext();
        			
        			//severs the link of this node to the next
        			currNode.setNext(null);
        			return true;
        		}
        		else
        		{
        			//removes the current node from the link and links the
        	...
        	//the value is not this current node
        	//moves on to the next node
        	//sets the current node to the previous node
        	previousNode = currNode;
        	
        	//get the next node in the list and make it the current node
        	currNode = currNode.getNext();
        }
    ...
    public int remove(int index, boolean dummy) throws IndexOutOfBoundsException{
        ...
        //set current node and point in index
        //Also setting values for the next node, and the next node to the next node
        //This is so we can remove them.
        Node currNode = mHead;
        Node nextNode;
        Node prevNode;
        int node = 0;
        ...
        //checks to see if the node we wish to remove has been found
        while (!found)
        {
        	//We are not yet at the index where we need to remove the node
        	if(node<index)
        	{
        		//Sets the current node to the previous node
        		...
        		//Checks to see if this is the final node in the block
        		//If it is it sets the getNext on the previous node to null
        		//otherwise it sets the getNext on the previous node to the next node
        		//and sets the get next on this node to 0
